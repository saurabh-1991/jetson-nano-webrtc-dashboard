...
docker run -d \
  ...
  -e CAMERA_BUFFER_FLUSH_GRABS=2 \
  -e CAMERA_DIRECT_V4L2_TUNE=true \
  -e CAMERA_SHARED_FRAME_PRODUCER_ENABLED=false \
  -e CAMERA_SHARED_FRAME_PRODUCER_IDLE_SECONDS=6 \
  -e CAMERA_SHARED_FRAME_WAIT_MS=260 \
  -e CAMERA1_AUTO_BRIGHTNESS=true \
  -e CAMERA2_AUTO_BRIGHTNESS=true \
  -e EXPERIMENTS_VIDEO_USE_SHARED_FRAME_CACHE=false \
  -e EXPERIMENTS_VIDEO_SHARED_MEMORY_STRICT=false \
  -e EXPERIMENTS_VIDEO_CACHE_MAX_AGE_SECONDS=0.45 \
  -e EXPERIMENTS_VIDEO_DIRECT_PULL_INTERVAL_SECONDS=0.9 \
  -e CUDA_ENABLED=true \
