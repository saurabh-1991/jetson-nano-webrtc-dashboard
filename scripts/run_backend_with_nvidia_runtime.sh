...
docker run -d \
  ...
  -e CAMERA_H264_STREAM_ENABLED="${CAMERA_H264_STREAM_ENABLED:-true}" \
  ...
  -e CAMERA1_H264_STREAM_ENABLED="${CAMERA1_H264_STREAM_ENABLED:-${CAMERA_H264_STREAM_ENABLED:-true}}" \
  -e CAMERA2_H264_STREAM_ENABLED="${CAMERA2_H264_STREAM_ENABLED:-${CAMERA_H264_STREAM_ENABLED:-true}}" \
  ...
  -e MODBUS_TRANSPORT="${MODBUS_TRANSPORT:-tcp}" \
  -e MODBUS_PORT="${MODBUS_PORT:-/dev/ttyUSB0}" \
  -e MODBUS_HOST="${MODBUS_HOST:-192.168.0.204}" \
  ...
  -e FLOW_METER_ENABLED="${FLOW_METER_ENABLED:-true}" \
  -e FLOW_METER_TRANSPORT="${FLOW_METER_TRANSPORT:-tcp}" \
  -e FLOW_METER_HOST="${FLOW_METER_HOST:-192.168.0.203}" \
  ...
  -e FLOW_METER_ADDRESS_BASE="${FLOW_METER_ADDRESS_BASE:-40001}" \
  -e FLOW_METER_ADDRESS_OFFSET="${FLOW_METER_ADDRESS_OFFSET:-0}" \
  -e FLOW_METER_VALUE_ADDRESS="${FLOW_METER_VALUE_ADDRESS:-40003}" \
  -e FLOW_METER_VALUE_REGISTER_COUNT="${FLOW_METER_VALUE_REGISTER_COUNT:-2}" \
  -e FLOW_METER_VALUE_ENCODING="${FLOW_METER_VALUE_ENCODING:-float32}" \
  -e FLOW_METER_WORD_ORDER="${FLOW_METER_WORD_ORDER:-ba}" \
  -e FLOW_METER_BYTE_ORDER="${FLOW_METER_BYTE_ORDER:-big}" \
  -e FLOW_METER_VELOCITY_ADDRESS="${FLOW_METER_VELOCITY_ADDRESS:-40005}" \
  -e FLOW_METER_VELOCITY_REGISTER_COUNT="${FLOW_METER_VELOCITY_REGISTER_COUNT:-2}" \
  -e FLOW_METER_DECIMAL_ADDRESS="${FLOW_METER_DECIMAL_ADDRESS:-}" \
  -e FLOW_METER_STATUS_ADDRESS="${FLOW_METER_STATUS_ADDRESS:-}" \
  -e FLOW_METER_SCALE="${FLOW_METER_SCALE:-1.0}" \
  ...
  -e FLOW_METER_VELOCITY_MIN_VALUE="${FLOW_METER_VELOCITY_MIN_VALUE:--99999.0}" \
  -e FLOW_METER_VELOCITY_MAX_VALUE="${FLOW_METER_VELOCITY_MAX_VALUE:-99999.0}" \
  ...
  -e VFD_ENABLED="${VFD_ENABLED:-true}" \
  -e VFD_HOST="${VFD_HOST:-192.168.0.202}" \
  ...
  -e VFD_ADDRESS_BASE="${VFD_ADDRESS_BASE:-0}" \
  -e VFD_ADDRESS_OFFSET="${VFD_ADDRESS_OFFSET:-0}" \
  -e VFD_RUN_COMMAND_REGISTER="${VFD_RUN_COMMAND_REGISTER:-8192}" \
  -e VFD_SPEED_COMMAND_REGISTER="${VFD_SPEED_COMMAND_REGISTER:-8193}" \
  -e VFD_RUN_FORWARD_WORD="${VFD_RUN_FORWARD_WORD:-18}" \
  -e VFD_STOP_WORD="${VFD_STOP_WORD:-1}" \
  -e VFD_MIN_WRITE_INTERVAL_MS="${VFD_MIN_WRITE_INTERVAL_MS:-150}" \
  -e VFD2_ENABLED="${VFD2_ENABLED:-true}" \
  -e VFD2_HOST="${VFD2_HOST:-192.168.0.201}" \
  -e VFD2_PORT="${VFD2_PORT:-502}" \
  -e VFD2_SLAVE_ID="${VFD2_SLAVE_ID:-1}" \
  -e VFD2_TIMEOUT_SECONDS="${VFD2_TIMEOUT_SECONDS:-1.0}" \
  -e VFD2_MIN_SPEED_HZ="${VFD2_MIN_SPEED_HZ:-0.0}" \
  -e VFD2_MAX_SPEED_HZ="${VFD2_MAX_SPEED_HZ:-50.0}" \
  -e VFD2_DEFAULT_SPEED_HZ="${VFD2_DEFAULT_SPEED_HZ:-0.0}" \
  -e VFD2_SPEED_SCALE="${VFD2_SPEED_SCALE:-100}" \
  -e VFD2_ADDRESS_BASE="${VFD2_ADDRESS_BASE:-0}" \
  -e VFD2_ADDRESS_OFFSET="${VFD2_ADDRESS_OFFSET:-0}" \
  -e VFD2_RUN_COMMAND_REGISTER="${VFD2_RUN_COMMAND_REGISTER:-8192}" \
  -e VFD2_SPEED_COMMAND_REGISTER="${VFD2_SPEED_COMMAND_REGISTER:-8193}" \
  -e VFD2_RUN_FORWARD_WORD="${VFD2_RUN_FORWARD_WORD:-18}" \
  -e VFD2_STOP_WORD="${VFD2_STOP_WORD:-1}" \
  -e VFD2_MIN_WRITE_INTERVAL_MS="${VFD2_MIN_WRITE_INTERVAL_MS:-150}" \
